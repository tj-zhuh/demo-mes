 ...
 */
        ...
        !(__WEBPACK_AMD_DEFINE_RESULT__ = function () {
            ...
            // 绘制元素，返回生成的dom元素
            function drawNode(menuId, menuName, level, iconUrl, iconActiveUrl) {
                ...
                if (level == 1 && iconUrl) {
                    ...
                    imgActive = document.createElement('img');
                    imgActive.src = iconActiveUrl ? iconActiveUrl : iconUrl;
                    imgActive.className = 'menu-active-icon';
                    li.appendChild(imgActive);
                }
            ...
            return function (ctns) {
                ...
                for (var i = 0; i < ctns.length; i++) {
                    ...
                    if (ctn.nodes.length == 0) {
                        ctn.outer.style.display = 'none';
                        ctn.dom.style.display = 'none';
                        continue;
                    }

                    ctn.outer.style.display = 'block';
                    ...
                    for (var j = 0; j < ctn.nodes.length; j++) {
                        ...
                        if (!node.dom) {
                            node.dom = drawNode(node.menuId, node.menuName, node.level, node.iconUrl, node.iconActiveUrl);
                            created.push(node);
                        }
                ...
                while (loopCount++ < maxCount) {
                    for (var i = 0; i < arr.length; i++) {
                        ...
                        if (parent) {
                            ...
                            node.iconActiveUrl = arr[i].iconActiveUrl;
        ...
        !(__WEBPACK_AMD_DEFINE_RESULT__ = function (require) {
            ...
            return build(function () {
                ...
                function ctor() {
                    ...
                    this.origin;    // 正在激活的叶子节点的origin
                    this.activeMenus;   // 正在激活的节点列表，每一项包含menuId、menuName、folder、origin等字段
                }
                ...
                ctor.prototype.init = function (_dom) {
                    ...
                    for (var i = 0; i < 4; i++) {
                        ...
                        outer.className = "mfr" + (i + 1) + "-outer";
                        _dom.appendChild(outer);
